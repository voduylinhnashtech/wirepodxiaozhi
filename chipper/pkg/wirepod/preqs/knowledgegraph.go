...
func streamingKG(req *vtt.KnowledgeGraphRequest, speechReq sr.SpeechRequest) string {
	...
	if strings.Contains(lowerText, "câu hỏi") || strings.Contains(lowerText, "question") {
		...
		logger.Println("(KG) Bot " + speechReq.Device + ": Conversation mode activated (detected 'câu hỏi' or 'question')")
	}
	_, err = ttr.StreamingKGSim(req, req.Device, transcribedText, true, isConversationMode)
	if err != nil {
